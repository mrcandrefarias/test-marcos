...
class User:
    ...
    def __init__(self, username, balance = None, credit_card = None):
    		
        self.credit_card_number = credit_card if self.add_credit_card(credit_card) else None
    ...
    def payment_validation(self, target, amount) -> float:
        amount = float(amount)
    ...
    def pay(self, target, amount, note):
        ...
        amount = self.payment_validation(target, amount)

        # TODO: add logic to pay with card or balance
        if self.balance >= amount:
            payment = self.pay_with_balance(target, amount)
        else:
            payment = self.pay_with_card(target, amount, note)

            
        self.payment_feeds = payment 
...
class MiniVenmo:
    ...
    def render_feed(self, feed):
        ...
        # TODO: add code here
        for payment in feed:
           feed_str = f"{payment.actor} paid {payment.target} ${payment.ammount} {payment.note} \n"
           print(feed_str)
